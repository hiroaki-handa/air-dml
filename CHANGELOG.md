...
### Changed
- SPECIFICATION.md: `swap_edge` の説明から「Mode-ai拡張」ラベルを削除
...
### Changed
- コードフェンスの言語識別子を ` ```dbml ` → ` ```airdml ` に統一（README・SPECIFICATION.md 計18箇所）
...
### Changed
- README・CHANGELOGをv2.1.9までの全変更内容に合わせて更新
- `values:` / `default:` / `hidden:` の構文をドキュメントに明記
- CLAUDE.mdにnpm publishルール（GitHub pushと必ずセット）を追記
