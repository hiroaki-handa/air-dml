...
**パラメータ**:
- `database_type`: `PostgreSQL` | `MySQL` | `SQLite` | `SQL Server` | `Oracle` | `BigQuery` | `Redshift` | `Snowflake`
...
| `fk` | Foreign Key（外部キー、Refと併用） | `user_id integer [fk]` |
| `unique` | 一意制約 | `email varchar(255) [unique]` |
| `not null` | NULL不可 | `username varchar(100) [not null]` |
| `increment` | 自動採番 | `id integer [pk, increment]` |
| `hidden` | カラムを非表示扱いにする | `MANDT varchar(3) [hidden]` |
| `alias: "論理名"` | カラムの論理名（日本語など） | `[alias: "ユーザーID"]` |
| `note: "説明"` | カラムの説明・補足 | `[note: "メールアドレス（重複不可）"]` |
| `values: "k=v/..."` | 区分値（列挙型の選択肢）を定義 | `[values: "1=有効/2=無効/3=削除済"]` |
| `default: "値"` | デフォルト値 | `[default: "active"]` |

#### `values:` 区分値の書式

ステータス・種別・フラグなど、取りうる値が決まっているカラムに使用します。
...
- `key=ラベル` 形式で値と表示名を対応付ける
- 複数の値は `/` で区切る
- キーなし（ラベルのみ）も許容: `values: "有効/無効"`
...
テーブルをグルーピングし、論理的な領域（境界付きコンテキスト）を定義します。
...
Table posts [alias: "投稿", pos_x: 100, pos_y: 450, color: "#9333EA"] {
  ...
  status varchar(1) [not null, alias: "公開ステータス", values: "1=公開/2=下書き/3=非公開", default: "2"]
  created_at timestamp [not null, alias: "作成日時"]
  updated_at timestamp [not null, alias: "更新日時"]
}
...
Table subscriptions [alias: "定期購入"] {
  ...
  status varchar(10) [not null, alias: "ステータス", values: "active=有効/paused=停止/cancelled=解約", default: "active"]
...
Table subscription_deliveries [alias: "定期購入配送履歴"] {
  ...
  status varchar(10) [not null, alias: "配送ステータス", values: "pending=未発送/shipped=発送済/delivered=配達完了", default: "pending"]
  created_at timestamp [not null, alias: "作成日時"]
  updated_at timestamp [not null, alias: "更新日時"]
}
...
| バージョン | 日付 | 変更内容 |
|------------|------|----------|
| 2.1.10 | 2026-05-10 | ドキュメント整備・仕様書を現バージョンに同期 |
| 2.1.9 | 2026-04-25 | `values:` / `default:` カラム属性追加、SAP R/3名前空間対応 |
| 2.1.8 | 2026-04-02 | `hidden` カラム属性追加 |
| 2.1.7 | 2026-04-02 | 属性キーワードをカラム名として使用可能に |
| 2.1.6 | 2026-03-30 | 予約語をカラム名として使用可能に |
| 2.1.5 | 2026-03-30 | スペースを含む型名のエクスポート修正 |
| 2.1.4 | 2026-03-29 | PostgreSQL SQL標準型名を追加 |
| 2.1.3 | 2026-03-28 | 全8DBの型リスト拡充 |
